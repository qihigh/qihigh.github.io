+++
date = "2015-10-26T15:40:23+08:00"
draft = true
title = "DNS域名绑定"
...
接下来介绍weico在DNSPod上购买的域名处理方式。
在weico.cc域名下得记录管理中，有添加的很多记录，这里首先是默认的三个@记录，这是默认的记录，不用管。
接下来列表中还有很多，比如abc对应的记录：

> + 主机记录:abc	--表示的是主机的三级域名
+ 记录类型:A	--表示的是A类记录
+ 线路类型:默认	--这里也包含海外和国内
+ 记录值:42.62.2.51 	--域名对应的ip地址
+ TTl:120		--过期时间，这里是两分钟

在这里，尝试ping 42.62.2.51 是可以ping通的，abc加上二级域名weico.cc，组成abc.weico.cc，这个应该也是可以ping通的。

印证一下
uploadify的地址是 http://118.26.225.125:8006/index， 
在记录值中找到对应这个ip的有几条记录，这里选用weicofile这个三级域名，将地址更新为 http://weicofile.weico.cc:8006/index， 发现访问的结果是相同的。
